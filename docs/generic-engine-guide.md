...
```bash
go run ./cmd/forge build generic-engine generic-test-runner
```
...
engines:
  - alias: <alias-name>              # Required: Unique identifier
    engine: <engine-uri>             # Required: go://generic-engine or go://generic-test-runner
    config:
...
**Values**:
- `go://generic-engine` - For build operations
...
**How it works**:
1. Forge resolves `alias://go-formatter` to `go://generic-engine`
2. Injects config (command, args) into MCP call
3. generic-engine executes: `gofmt -l -w .`
...
When generic-engine/generic-test-runner executes your command:
...
# 2. Test generic-engine directly
./build/bin/generic-engine --help
...
| Aspect | generic-engine | generic-test-runner |
|--------|----------------|---------------------|
...
engines:
  - alias: string                    # Required, unique
    engine: string                   # Required, go://generic-engine or go://generic-test-runner
    config:
...
- Check `cmd/generic-engine` and `cmd/generic-test-runner` source code for implementation details
